...
public:
    denselayer (const int inChannels, const int NumChannels):
        ...
        torch::Tensor fwd(const torch::Tensor& input)
        {
            return layer_net->fwd(input);
        }
...
public:
    densenet121()
    {
        ...
        network->puch_back(torch::nn::BatchNorm2d(torch::nn::BatchNorm2dOptions(64)));
